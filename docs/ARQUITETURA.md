...
## 4. O que NÃO está no MVP (por decisão de escopo, não por esquecimento)

Conforme o roadmap fornecido, ficam para fases seguintes — e a interface
sinaliza isso explicitamente, em vez de simular funcionalidade inexistente:

- **Diarização** (separação de participantes) — Fase 2.
- **Dicionário institucional de termos** — Fase 2.
...
- **Desempenho em CPU**: reuniões de 2h+ podem levar bastante tempo em
  ...
  interface) — mas o usuário deve ter essa expectativa de tempo.
- **Qualidade de diarização local**: soluções open source ainda ficam atrás
  de serviços comerciais. Por isso a Fase 1 não afirma "identificar
  participantes" — apenas na Fase 2, com aviso claro de que é heurística.
...
## 7. Estratégia de segurança (MVP → evolução)
